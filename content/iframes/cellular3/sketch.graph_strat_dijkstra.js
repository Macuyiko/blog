...
function setup() {
  ...
  nodes.push(new Node(300, 40));
  nodes.push(new Node(300, 120));
  nodes.push(new Node(500, 120));
  addEdge(3, 8); addEdge(3, 9); addEdge(9, 10); addEdge(5, 10);
...
function draw() {
  ...
  for (var i = 0; i < packets.length; i++) {
    ...
    if (packets[i].trajectory)
      text('Moving to ' + packets[i].to + ' | Next steps: ' + packets[i].trajectory.join(", "), packets[i].x-5, packets[i].y-10);
  }
