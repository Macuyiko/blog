...
The idea works as follows. Say you've just trained a model using a feature set X, row/column indexed as X[i,j]. To assess the importance of a variable j, we can compare a metric of interest (accuracy or AUC, or anything else really) on X with a modified instance of X, X', where we permute, i.e. shuffle, the column j.
...
scikit-learn doesn't implement permutation based importance rankings, but using `rfpimp` yields a similar result:
...
Knowing which features play an important role is helpful (disregarding the interaction aspect for a bit). But the next question one will be asked is commonly "why" or "how" these are important. An easy mistake to make is to think linearly and to think that if x goes up, y will go up (or down), though this ignores all those non-linear effects these black-box models are so good at extracting.
...
- In any case, we ask the model to predict a probability for each row in our synthetic data set (non-considered variables are simply constant for all k =/= j across the rows) and plot the results
...
Second, we can stil easily extract a general "partial dependence style" plot by averaging the value across the lines, as displayed by the center yellow line in the picture above.
...
- H2(jk) to measure wether features j and k interact. For this, partial dependence results need to be calculated for j and k seperately, and j and k together. I'll call this the second-order measure
...
Especially in Python, the situation regarding the H-measure looks a bit bleak. Using `pdpbox` to extract partial dependence values, we can however set up an implementation of both measures which work for any model. For two-way interactions (hiding the exact calculation of the H-measure until a bit later), we can compare our result with what `sklearn_gbmi` tells us for H2(jk):
	...
	gradient boosting (first result is `sklearn_gbmi`):
...
However, for H2(jkl) and higher-order interactions, we need to modify `pdpbox` so that is is able to calculate partial dependence values for more than two variables under consideration. In fact, `pdpbox` does accept more than two features when calling it, but will simply ignore everything apart from the first two.
...
Testing this on one of our models, we get:

	x0 is in interaction: 0.2688984990347688
	x1 is in interaction: 0.26778914050926556
	x2 is in interaction: 0.10167180905369481

We can then calculate the second-order H measure for H2(x0,...) and H2(x1,...), also for higher-order interactions:
...
In my search on "white washing" black box models, I discovered the following clever approaches:
...
- [`skope-rules`](https://github.com/scikit-learn-contrib/skope-rules): a trade off between the interpretability of a Decision Tree and the modelization power of a Random Forest, which offers a very interesting approach as well aimed at deriving a short list of precise rules
