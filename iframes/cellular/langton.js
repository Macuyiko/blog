...
Grid.prototype.draw = function() {
  ...
  this.visit(function(col, row) {
    ...
    if (cell.ant) {
      ...
      fill(color(255,0,0));
      ellipse(col*self.size + floor(self.size/2), 
              row*self.size + floor(self.size/2), 5);
    }
  });
}
...
Grid.prototype.neighborhood = function(col, row) {
  ...
  n.left = col > 0 ? this.cells[col-1][row] : this.cells[this.cols-1][row];
  n.right = col < (this.cols - 1) ? this.cells[col+1][row] : this.cells[0][row];
  n.up = row > 0 ? this.cells[col][row-1] : this.cells[col][this.rows-1];
  n.down = row < (this.rows -1) ? this.cells[col][row+1] : this.cells[col][0];
  return n;
}
